...
rec {
  ...
  writeShellScriptBinWithArgs = { drv, bin-name ? pname-or-name drv, args ? [ ], append-args ? true, set-eux ? true }: let
    name' = if append-args then (replace-slash (builtins.concatStringsSep "-" ([ (prefix + bin-name) ] ++ args))) else bin-name;
  ...
  drvToAppWithArgs = { drv, bin-name ? pname-or-name drv, args ? [ ] }: let
