...
let
	...
	letsql-asciinema-rec-local = pkgs.writeShellScriptBin "${prefix}asciinema-rec-local" ''
		set -eux
  ...
  letsql-debug-drv-setup-script = pkgs.writeText "${prefix}debug-drv-setup.sh" ''
...
in {
	inherit commands commands-star commands-shell;
}
